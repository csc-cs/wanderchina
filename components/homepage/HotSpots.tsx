...
export function HotSpots() {
      ...
      <section className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8 py-16" aria-label="Popular attractions">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4 md:gap-6">
          {[1, 2, 3, 4, 5, 6, 7, 8].map((i) => (
            <div key={i} className="aspect-[3/4] bg-slate-100 animate-pulse rounded-2xl motion-reduce:animate-none" />
          ))}
        </div>
      </section>
    );
      ...
      <section className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8 py-16">
        ...
        <div className="rounded-2xl border border-slate-200 bg-brand-surface p-8 text-center">
          ...
          <button
            type="button"
            onClick={() => {
              console.warn('[HotSpots] retrying fetch');
              refetch();
            }}
            className="rounded-full bg-brand-primary text-white px-5 py-2 text-sm font-medium hover:bg-indigo-700 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-brand-primary focus-visible:ring-offset-2"
          >
            Retry
          </button>
        </div>
  ...
  // Empty state is distinct from error state — silently empty data is a valid outcome
  if (spots.length === 0) {
    return (
      <section className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8 py-16">
        <SectionHeader />
        <div className="rounded-2xl border-2 border-dashed border-slate-200 bg-brand-surface p-10 text-center">
          ...
          <p className="text-sm text-brand-muted mb-5">
            Check back soon as more travelers explore.
          </p>
          <Link
            href="/guides"
            className="inline-flex items-center gap-2 rounded-full bg-brand-primary text-white px-5 py-2.5 text-sm font-medium hover:bg-indigo-700 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-brand-primary focus-visible:ring-offset-2"
          >
  ...
  return (
    <section className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8 py-16" aria-label="Popular attractions">
      ...
      <div className="md:hidden flex gap-4 overflow-x-auto snap-x snap-mandatory no-scrollbar pb-2">
        {spots.map((spot) => (
          <div key={spot.spot_id} className="flex-shrink-0 w-[70vw]">
...
function SpotCard({ spot, compact = false }: { spot: HotSpotItem; compact?: boolean }) {
    ...
    >
      {imgError ? (
        ...
        <Image
          ...
          alt={spot.name_en}
          fill
          sizes={compact ? '70vw' : '(max-width: 1024px) 50vw, 25vw'}
          className="object-cover group-hover:scale-110 transition-transform duration-500 motion-reduce:transition-none motion-reduce:group-hover:scale-100"
          onError={() => {
            console.warn(`[HotSpots] Cover image failed for spot: ${spot.spot_id}`);
            setImgError(true);
          }}
        />
      )}
      ...
      <div className="absolute bottom-0 left-0 right-0 p-4 text-white">
        <h3 className={`font-bold mb-1 line-clamp-1 ${compact ? 'text-sm' : 'text-base sm:text-lg'}`}>
          {spot.name_en}
        </h3>
